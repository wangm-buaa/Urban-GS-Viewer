...
__global__ void rasterize_to_pixels_bwd_kernel(
    ...
    const S *__restrict__ gs_masks,      // [C, N]
    ...
    S *__restrict__ v_gs_masks          // [C, N] or [nnz]
) {
    ...
    S* gs_masks_batch = reinterpret_cast<S*>(&conic_batch[block_size]); // [block_size]

    S *rgbs_batch = (S *)&gs_masks_batch[block_size]; // [block_size * COLOR_DIM]
...
//         if (isnan(v_render_c[k])) {
//             printf("bwd Warning: v_render_c=%.10f, camera_id=%.10f,\n", v_render_c[k], camera_id);
//         }
    }
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if (valid) {
                ...
                vis = __expf(-sigma);
                alpha = min(0.999f, opac * vis) * gs_mask;
//                 if (gs_mask < 1.f){
//                     printf("bwd Warning: gs_mask=%.5f, alpha=%.5f\n", gs_mask, alpha);
//                 }
                if (sigma < 0.f || (opac * vis) < 1.f / 255.f){
                    valid = false;
                }
            }
            ...
            if (valid) {
                ...
                if (isnan(v_alpha)) {
                    printf("bwd Warning: pix_id=%d\n", pix_id);
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        printf("bwd Warning: v_render_colors[%d]=%.10f\n", k, v_render_colors[pix_id * COLOR_DIM + k]);
                    }
                    for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                        printf("bwd Warning: k=%d, rgbs_batch=%.10f, buffer=%.10f, v_render_c=%.10f, backgrounds=%.10f\n", k, rgbs_batch[t * COLOR_DIM + k],  buffer[k], v_render_c[k], backgrounds[k]);
                    }
                    printf("bwd Warning: T=%.10f, T_final=%.10f, v_render_a=%.10f, ra=%.5f\n", T, T_final, v_render_a, ra);
                }

                if (opac * vis <= 0.999f) {
                    const S v_sigma = -opac * vis * v_alpha * gs_mask;
//                     const S v_sigma = -opac * vis * v_alpha;
                    ...
                    v_opacity_local = vis * v_alpha * gs_mask;
//                     v_opacity_local = vis * v_alpha;
                    v_gs_mask_local = vis * v_alpha * opac;
                }

//                 if (isnan(v_alpha) || isnan(v_opacity_local) || isnan(v_gs_mask_local) || isnan(v_conic_local.x) || isnan(v_xy_local.x)) {
//                     printf("bwd Warning: v_alpha=%.10f, v_opacity_local=%.10f, v_gs_mask_local=%.10f, v_conic_local=%.10f, v_xy_local=%.10f\n", v_alpha, v_opacity_local, v_gs_mask_local, v_conic_local.x, v_xy_local.x);
// //                     printf("bwd Warning: vis=%.10f, opac=%.10f, dL_mask=%.10f,\n", vis, opac, );
//                 }

//                 if (gs_mask < 1.f){
//                     printf("bwd Warning: gs_mask=%.5f, dL_dalpha=%.10f, dL_mask=%.10f,\n", gs_mask, v_alpha, v_gs_mask_local);
//                 }
            ...
            warpSum<decltype(warp), S>(v_gs_mask_local, warp);
            ...
            if (warp.thread_rank() == 0) {
                ...
                gpuAtomicAdd(v_gs_masks + g, v_gs_mask_local);
            }
        }
    }
}
...
call_kernel_with_dim(
    ...
    const torch::Tensor &gs_masks,       // [C, tile_height, tile_width]
    // image size
    const uint32_t image_width, const uint32_t image_height, const uint32_t tile_size,
    // intersections
    ...
    torch::Tensor v_gs_masks = torch::zeros_like(gs_masks);       // [C, tile_height, tile_width]
    ...
    if (n_isects) {
        ...
        rasterize_to_pixels_bwd_kernel<CDIM, float>
            <<<blocks, threads, shared_mem, stream>>>(
                ...
                v_gs_masks.data_ptr<float>());
    }

    return std::make_tuple(v_means2d_abs, v_means2d, v_conics, v_colors, v_opacities, v_gs_masks);
}
...
rasterize_to_pixels_bwd_tensor(
    ...
    const at::optional<torch::Tensor> &backgrounds, // [C, 3]
    const at::optional<torch::Tensor> &masks,       // [C, tile_height, tile_width]
    const torch::Tensor &gs_masks,       // [C, N] or [nnz]
    // image size
    const uint32_t image_width, const uint32_t image_height, const uint32_t tile_size,
    // intersections
    const torch::Tensor &tile_offsets, // [C, tile_height, tile_width]
...
#define __GS__CALL_(N)                                                                 \
    case N:                                                                            \
        return call_kernel_with_dim<N>(                                                \
            means2d, conics, colors, opacities, backgrounds, masks, gs_masks, image_width,       \
