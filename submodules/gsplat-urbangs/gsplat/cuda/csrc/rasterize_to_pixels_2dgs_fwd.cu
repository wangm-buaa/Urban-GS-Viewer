 ...
 __global__ void rasterize_to_pixels_fwd_2dgs_kernel(
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            int32_t weight_cur = static_cast<int>(vis * 1000);
...
call_kernel_with_dim(
    ...
    rasterize_to_pixels_fwd_2dgs_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            median_ids.data_ptr<int32_t>(), weights.data_ptr<int32_t>());
    
    return std::make_tuple(renders, alphas, render_normals, render_distort, render_median, last_ids, median_ids, weights);
}
