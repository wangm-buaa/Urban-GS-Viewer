...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    int32_t *__restrict__ weights, // [C, N, 1]
    int32_t *__restrict__ pixels // [C, N, 1]
) {
    ...
    S* gs_masks_batch = reinterpret_cast<S*>(&conic_batch[block_size]);
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            const S gs_mask = gs_masks_batch[t];
            const vec2<S> delta = {xy_opac.x - px, xy_opac.y - py};
            const S sigma =
                0.5f * (conic.x * delta.x * delta.x + conic.z * delta.y * delta.y) +
                conic.y * delta.x * delta.y;
            S alpha = min(0.999f, opac * __expf(-sigma)) * gs_mask;

//             if (gs_mask < 1.f){
//                 printf("fwd Warning: gs_mask=%.5f, alpha=%.5f\n", gs_mask, alpha);
//             }
            if (sigma < 0.f || (opac * __expf(-sigma)) < 1.f / 255.f) {
                continue;
            }
...
//             float weight_cur = alpha * T;
//             if (weight_cur > 1.0f){
//                 printf("Warning: alpha=%.5f, T=%.5f, weight=%.5f\n", alpha, T, weight_cur);
//             }
            int32_t weight_cur = static_cast<int>(vis * 10000);
//             if (weight_cur > 0){
//                 printf("Warning: alpha=%.5f, T=%.5f, weight=%d， weights_store=%d\n", alpha, T, weight_cur, weights[id_batch[t]]);
//             }
            atomicMax(&weights[id_batch[t]], weight_cur);
            atomicAdd(&(pixels[id_batch[t]]), 1);
            T = next_T;
        }
    }
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> call_kernel_with_dim(
    ...
    torch::Tensor alphas = torch::empty({C, image_height, image_width, 1},
                                        means2d.options().dtype(torch::kFloat32));
    torch::Tensor weights = torch::zeros({C, N, 1},
                                        means2d.options().dtype(torch::kInt32));
    torch::Tensor pixels = torch::zeros({C, N, 1},
                                        means2d.options().dtype(torch::kInt32));
    ...
    rasterize_to_pixels_fwd_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            last_ids.data_ptr<int32_t>(), weights.data_ptr<int32_t>(), pixels.data_ptr<int32_t>());

    return std::make_tuple(renders, alphas, last_ids, weights, pixels);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> rasterize_to_pixels_fwd_tensor(
    ...
    const at::optional<torch::Tensor> &backgrounds, // [C, channels]
    const at::optional<torch::Tensor> &masks,       // [C, tile_height, tile_width]
    const torch::Tensor &gs_masks,       // [C, N]
    // image size
    const uint32_t image_width, const uint32_t image_height, const uint32_t tile_size,
    // intersections
    const torch::Tensor &tile_offsets, // [C, tile_height, tile_width]
...
#define __GS__CALL_(N)                                                                 \
    case N:                                                                            \
        return call_kernel_with_dim<N>(means2d, conics, colors, opacities,             \
                                       backgrounds, masks, gs_masks, image_width, image_height,  \
