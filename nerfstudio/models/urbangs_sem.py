...
from nerfstudio.fields.urbangs_sem_field import GaussianSplattingField
...
class GaussianSplatting(Model):
    ...
    def populate_modules(self):
        ...
        loaded_iter = searchForMaxIteration(os.path.join(self.geo_base_path, "point_cloud"))
        self.gaussian_model.load_ply(os.path.join(self.geo_base_path,
                                                  "point_cloud",
                                                  "iteration_" + str(loaded_iter),
                                                  "point_cloud.ply"))
        self.gaussian_model.load_mlp_checkpoints(os.path.join(self.geo_base_path,
                                                            "point_cloud",
                                                            "iteration_" + str(loaded_iter)))
        self.gaussian_model.load_sem_weights(os.path.join(self.model_path,
                                                  "point_cloud",
    ...
    def get_outputs_for_camera_ray_bundle(self, camera_ray_bundle: RayBundle) -> Dict[str, torch.Tensor]:
        ...
        semantic_info = render_results["sem_feat"]
        ...
        semantic_info = torch.permute(torch.clamp(semantic_info, max=1.), (1, 2, 0))
        return {
            "rgb": rgb,
            "depth": vis_depth,
    ...
    def render(viewpoint_camera, pc, pipe, bg_color: torch.Tensor, scaling_modifier=1.0, override_color=None):
        ...
        xyz, offset, color, opacity, scaling, rot, sh_degree, selection_mask, language_feature_precomp, anchor_indices = pc.generate_neural_gaussians(
        ...
        feat = torch.cat([color, language_feature_precomp[:, :3]], dim=-1)
        ...
        colors, sem_feat, depths = render_colors[..., 0:3], render_colors[..., 3:6], render_colors[..., 6:7]

        rendered_image = colors[0].permute(2, 0, 1)
        depth = depths[0].permute(2, 0, 1)
        sem_feat = sem_feat[0].permute(2, 0, 1).contiguous()
        # print("sem_feat shape:", sem_feat.shape, "depth shape:", depth.shape, "color shape:", render_colors.shape)
