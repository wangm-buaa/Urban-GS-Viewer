...
class RenderStateMachine(threading.Thread):
    ...
    def _calculate_image_res(self, aspect_ratio: float) -> Tuple[int, int]:
        ...
        if self.state == "high":
            ...
            # Always render at the full viewport resolution, even while moving.
            # Original adaptive-resolution logic is kept below for easy restore.
            image_height = max_res
            image_width = int(image_height * aspect_ratio)
            if image_width > max_res:
                image_width = max_res
                image_height = int(image_width / aspect_ratio)
            # Original adaptive-resolution logic:
