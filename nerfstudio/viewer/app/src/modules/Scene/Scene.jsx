...
export function get_scene_tree() {
  ...
  camera_controls.minPolarAngle = 0.01;
  camera_controls.maxPolarAngle = Math.PI - 0.01;
  ...
  const viewRotSpeed = 0.004;
  const rollRotSpeed = 0.008;
  const EPS = 0.005;


  function rotate() {
    const yaw =
      (keyMap.KeyJ === true ? viewRotSpeed : 0) +
      (keyMap.KeyL === true ? -viewRotSpeed : 0);
    const pitch =
      (keyMap.KeyI === true ? viewRotSpeed : 0) +
      (keyMap.KeyK === true ? -viewRotSpeed : 0);
    const roll =
      (keyMap.KeyU === true ? -rollRotSpeed : 0) +
      (keyMap.KeyO === true ? rollRotSpeed : 0);
  ...
  function rotateView(yawAngle, pitchAngle) {
    const camera = camera_controls.camera;
    const position = camera_controls.getPosition(new THREE.Vector3());
    ...
    if (yawAngle !== 0) {
      forward.applyAxisAngle(screenUp, yawAngle).normalize();
    }

    if (pitchAngle !== 0) {
      const right = new THREE.Vector3()
        .crossVectors(forward, screenUp)
        .normalize();
      const pitchedForward = forward
        .clone()
        .applyAxisAngle(right, pitchAngle)
        .normalize();
      const cameraUp = camera.up.clone().normalize();
      const maxViewUpDot = Math.cos(camera_controls.minPolarAngle);

      if (Math.abs(pitchedForward.dot(cameraUp)) < maxViewUpDot) {
        forward.copy(pitchedForward);
      }
    }

    const target = position.clone().add(forward.multiplyScalar(EPS));
  ...
  function rollCamera(rollAngle) {
    const camera = camera_controls.camera;
    const position = camera_controls.getPosition(new THREE.Vector3());
    const target = camera_controls.getTarget(new THREE.Vector3());
    ...
    camera.up.applyAxisAngle(forward, rollAngle).normalize();
    camera_controls.updateCameraUp();
    camera_controls.setLookAt(
      position.x,
      position.y,
      position.z,
